...
This is not a slogan. It is a statement about which failure modes matter, and it
is the reason MACH's design looks nothing like a conventional price feed.
...
*did this specific account receive this specific amount?* One institution — the
one holding the account — knows definitively. Everyone else is guessing.
...
asking "has it arrived yet?" on a timer, the anchor pushes a notification the
moment the credit posts. Sub-second signal, no polling budget, no interval during
which a payment has settled but the protocol has not noticed.
...
| --- | --- | --- |
| The anchor's `SIGNING_KEY` in `stellar.toml` is authentic. | Served over TLS from a domain the anchor controls; the same assumption every SEP-10 authentication already makes. | An attacker controlling the anchor's DNS or hosting could forge notifications. Mitigated by pinning known anchor keys per deployment. |
| The anchor reports credits honestly. | The anchor is a regulated custodian holding the fiat; misreporting is a licensing matter, not merely a protocol one. | A malicious anchor could signal a payment that did not occur — the same exposure any fiat on-ramp carries. |
...
against the anchor's key. It can, at worst, **fail to relay** — a liveness
failure, not a safety one, and one the liquidation time-lock in the contract is
designed to absorb.

This is the property that distinguishes a payment oracle worth building from a
...
* **Replay protection is mandatory.** Since a single signed message authorises
  settlement, that message must be bound to one invoice and consumed exactly once.
  The contract enforces this; see
...
The next page walks the full path from provisioning to execution.
